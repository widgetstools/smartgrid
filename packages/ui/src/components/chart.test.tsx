...
import { ChartContainer, ChartLegendContent, ChartStyle, ChartTooltipContent } from './chart.js';

vi.mock('recharts', async (importOriginal) => {
  // eslint-disable-next-line @typescript-eslint/consistent-type-imports -- vi.mock pattern
...
describe('ChartContainer', () => {
  ...
  it('omits inline theme styles when no colors are configured', () => {
    const { container } = render(<ChartStyle id="empty-chart" config={{ plain: { label: 'Plain' } }} />);
...
describe('ChartTooltipContent', () => {
  it('returns nothing when inactive', () => {
    const { container } = renderInChartContext(<ChartTooltipContent active={false} payload={[]} />);
