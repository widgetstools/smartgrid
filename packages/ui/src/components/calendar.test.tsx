...
describe('Calendar', () => {
  it('renders a grid of days for the selected month', () => {
    ...
    expect(screen.getByRole('button', { name: 'Wednesday, July 15th, 2026' })).toBeInTheDocument();
  });
  ...
  it('selects a day when clicked', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Friday, July 10th, 2026' }));
  ...
  it('merges a caller className on the root picker', () => {
    const { container } = render(<Calendar className="shadow-lg" defaultMonth={new Date(2026, 6, 1)} />);
