...
describe('Slider', () => {
  it('updates value when a thumb is moved with the keyboard', async () => {
    ...
    render(<Slider aria-label="Opacity" defaultValue={[25]} max={100} onValueChange={onValueChange} />);
  ...
  it('does not respond while disabled', async () => {
    const onValueChange = vi.fn();
    render(
      <Slider aria-label="Locked" defaultValue={[25]} disabled max={100} onValueChange={onValueChange} />,
    );
