// ─────────────────────────────────────────────────────────────
//  SmartGrid design system — Vanilla TS AG Grid Cell Renderers
//  Origin: stern-bak packages/design-system/src/cellRenderers.ts.
...
//  (css/tokens.css) so renderers automatically adapt
  ...
  init(params: ICellRendererParams) {
    ...
    this.eGui = el(
      'span',
      {
        ...
        fontFamily: MONO,
        color: isBuy ? 'var(--sg-accent-positive)' : 'var(--sg-accent-negative)',
      },
      isBuy ? 'BUY' : 'SELL',
    );
  }
...
const STATUS_STYLES: Record<string, { bg: string; color: string; border: string }> = {
  Filled: {
    bg: 'var(--sg-overlay-positive-soft)',
    color: 'var(--sg-accent-positive)',
    border: 'var(--sg-overlay-positive-ring)',
  },
  Partial: {
    bg: 'var(--sg-overlay-warning-soft)',
    color: 'var(--sg-accent-warning)',
    border: 'var(--sg-overlay-warning-ring)',
  },
  Pending: {
    bg: 'var(--sg-overlay-info-soft)',
    color: 'var(--sg-accent-info)',
    border: 'var(--sg-overlay-info-ring)',
  },
  Cancelled: {
    bg: 'var(--sg-overlay-negative-soft)',
    color: 'var(--sg-accent-negative)',
    border: 'var(--sg-overlay-negative-ring)',
  },
  Working: {
    bg: 'var(--sg-overlay-info-soft)',
    color: 'var(--sg-accent-info)',
...
export class ColoredValueRenderer implements ICellRendererComp {
  ...
  init(params: ICellRendererParams) {
    ...
    const color = v >= 0 ? 'var(--sg-accent-positive)' : 'var(--sg-accent-negative)';
    const prefix = v > 0 ? '+' : '';
...
export class OasValueRenderer implements ICellRendererComp {
  ...
  init(params: ICellRendererParams) {
    const v = Number(params.value);
    const color = v > 80 ? 'var(--sg-accent-warning)' : 'var(--sg-accent-positive)';
...
export class SignedValueRenderer implements ICellRendererComp {
  ...
  init(params: ICellRendererParams) {
    ...
    this.eGui = el(
      'span',
      { fontFamily: MONO, color: 'var(--sg-text-secondary)' },
      `${prefix}${params.valueFormatted || params.value}`,
    );
  }
  getGui() {
    return this.eGui;
...
export class FilledAmountRenderer implements ICellRendererComp {
  ...
  init(params: ICellRendererParams) {
    ...
    const color = filled === qty ? 'var(--sg-accent-positive)' : 'var(--sg-accent-warning)';
...
export class BookNameRenderer implements ICellRendererComp {
  private eGui!: HTMLElement;
  init(params: ICellRendererParams) {
    this.eGui = el('span', { fontFamily: MONO, color: 'var(--sg-accent-highlight)' }, params.value);
  }
  getGui() {
    return this.eGui;
  }
...
export class YtdValueRenderer implements ICellRendererComp {
  ...
  init(params: ICellRendererParams) {
    ...
    const color = isPositive ? 'var(--sg-accent-positive)' : 'var(--sg-accent-negative)';
...
const RFQ_STATUS_STYLES: Record<string, { bg: string; color: string; border: string }> = {
  ...
  stale: {
    bg: 'var(--sg-overlay-neutral-soft)',
    color: 'var(--sg-text-muted)',
    border: 'var(--sg-overlay-neutral-ring)',
  },
};
    ...
    themeObserver = new MutationObserver(() => {
      ...
      for (const cb of themeSubscribers) {
        try {
          ...
          /* one broken cell must not break the rest */
        }
      }
    });
...
export class PillCellRenderer implements ICellRendererComp {
  ...
  private paint() {
    ...
    if (cfg) {
      ...
      chosen = match ? { bg: match.bg, fg: match.fg, border: match.border } : cfg.fallback;
    }
...
function pickPillCfg(params: ICellRendererParams & PillRendererConfig): PillRendererConfig | undefined {
...
export class HeatmapCellRenderer implements ICellRendererComp {
  ...
  private paint(params: ICellRendererParams) {
    ...
    const text =
      ...
      (Number.isFinite(n) ? String(n) : this.value == null ? '' : String(this.value));
    this.eGui.textContent = text;
    if (!cfg || !Number.isFinite(n)) {
      this.eGui.removeAttribute('style');
      return;
    }
...
function pickPercentBarCfg(
  p: ICellRendererParams & PercentBarRendererConfig,
): PercentBarRendererConfig | undefined {
...
export class TrendArrowCellRenderer implements ICellRendererComp {
  ...
  private paint() {
    ...
    const mode = readThemeMode();
    const direction: 'up' | 'down' | 'flat' = n > threshold ? 'up' : n < -threshold ? 'down' : 'flat';
    ...
    const color =
      direction === 'up'
        ...
        : direction === 'down'
          ...
          : pickThemeColor(cfg.neutralColor, mode) || 'var(--sg-text-muted)';
...
function pickTrendCfg(
  p: ICellRendererParams & TrendArrowRendererConfig,
): TrendArrowRendererConfig | undefined {
...
function pickSparklineCfg(
  p: ICellRendererParams & SparklineRendererConfig,
): SparklineRendererConfig | undefined {
...
export class MultiLineCellRenderer implements ICellRendererComp {
  ...
  private paint() {
    ...
    Object.assign(sec.style, {
      ...
      color: pickThemeColor(cfg.secondaryColor, mode) || 'var(--sg-text-muted)',
      fontFamily: MONO,
    } as Partial<CSSStyleDeclaration>);
...
function pickMultiLineCfg(
  p: ICellRendererParams & MultiLineRendererConfig,
): MultiLineRendererConfig | undefined {
...
export class IconTextCellRenderer implements ICellRendererComp {
  ...
  private paint() {
    ...
    if (cfg?.iconSvg) {
      ...
      // @smartgrid/design-system/icons `MARKET_ICON_SVGS`). Drop it in and force
...
function pickIconTextCfg(
  p: ICellRendererParams & IconTextRendererConfig,
): IconTextRendererConfig | undefined {
...
function pickCountryFlagCfg(
  p: ICellRendererParams & CountryFlagRendererConfig,
): CountryFlagRendererConfig | undefined {
...
function pickRatingDeltaCfg(
  p: ICellRendererParams & RatingDeltaRendererConfig,
): RatingDeltaRendererConfig | undefined {
...
function pickTimeSinceCfg(
  p: ICellRendererParams & TimeSinceRendererConfig,
): TimeSinceRendererConfig | undefined {
  return {
...
function pickAllocationBarCfg(
  p: ICellRendererParams & AllocationBarRendererConfig,
): AllocationBarRendererConfig | undefined {
