// ─────────────────────────────────────────────────────────────
//  SmartGrid design system — Cell Renderer Registry
//  Origin: stern-bak packages/design-system/src/cellRendererRegistry.ts.
//
 ...
 * `min` / (optional) `mid` / `max` colours. `domain` is optional and
 ...
 * `@smartgrid/design-system/icons`) at write time and stores both the source
 ...
 * render.
 */
export const cellRendererComponents: Readonly<Record<CellRendererId, new () => ICellRendererComp>> =
  Object.freeze({
    // Configurable
...
export const cellRendererCatalogue: ReadonlyArray<CellRendererCatalogueEntry> = Object.freeze([
  ...
  },
  {
    id: 'icon-text',
    label: 'Icon + Text',
    description: 'Icon from @smartgrid/design-system/icons next to the value.',
    category: 'composite',
    configurable: true,
  },
  {
    ...
    label: 'Country Flag',
    description:
      'Flag emoji from an ISO-3166-alpha-2 country code OR an ISO-4217 currency code (USD → 🇺🇸, EUR → 🇪🇺, JPY → 🇯🇵, …), plus optional label.',
    category: 'composite',
    configurable: true,
  },
...
export function getCellRendererEntry(id: string | undefined): CellRendererCatalogueEntry | undefined {
