// ─────────────────────────────────────────────────────────────
//  SmartGrid design system — Semantic Tokens
//  Origin: stern-bak packages/design-system/src/tokens/semantic.ts (StarUI).
//  Three themes: dark (Graphite), light clinical, light paper.
//  Canonical hex: tokens/staruiHex.ts (sgHex)
// ─────────────────────────────────────────────────────────────

import { colors, typography, radius, spacing, opacity, transition, shadow } from './primitives';
import { sgHex, buildShadcnFromSgHex, type SgHexPack } from './staruiHex';

export type ShadcnTokenPack = ReturnType<typeof buildShadcnFromSgHex>;
...
function schemeFromHex(h: SgHexPack, shadcn: ShadcnTokenPack, mode: 'dark' | 'light'): ColorScheme {
...
export const dark: ColorScheme = schemeFromHex(sgHex.dark, buildShadcnFromSgHex(sgHex.dark), 'dark');

/** Default light theme — "clinical". */
export const light: ColorScheme = schemeFromHex(
  sgHex.lightClinical,
  buildShadcnFromSgHex(sgHex.lightClinical),
  'light',
);

/** "paper" light variant — warm cream chrome. */
export const lightPaper: ColorScheme = schemeFromHex(
  sgHex.lightPaper,
  buildShadcnFromSgHex(sgHex.lightPaper),
  'light',
);
