// ─────────────────────────────────────────────────────────────
//  SmartGrid design system — canonical hex token packs
//  Origin: stern-bak packages/design-system/src/tokens/staruiHex.ts (StarUI);
//  exported identifiers renamed starui* → sg*. Source palette: css/tokens.css.
//  Cyan signature accent · mint-teal buy/up · rose sell/down.
// ─────────────────────────────────────────────────────────────

import { hexToHslChannel } from '../internal/wcag';

/** Raw surface + semantic hex pack (maps to --sg-* in CSS). */
export interface SgHexPack {
  bg: string;
  bg1: string;
  bg2: string;
  bg3: string;
...
export const sgHex = {
  dark: {
    ...
    shadowOverlay:
      '0 1px 0 rgba(255,255,255,0.05) inset, 0 8px 24px rgba(0,0,0,0.55), 0 2px 6px rgba(0,0,0,0.4)',
...
} as const satisfies Record<string, SgHexPack>;

/** shadcn HSL channel triplets derived from STARUI hex. */
export function buildShadcnFromSgHex(h: SgHexPack) {
...
/** AG Grid v33+ color pack — aligned with the AG Grid adapter token mapping. */
export function buildAgGridFromSgHex(h: SgHexPack, mode: 'dark' | 'light') {
  return {
    bg: h.bg1,
    fg: h.t0,
    chrome: h.bg2,
