...
import { contrastRatio } from '../internal/wcag';
...
describe('FI Design System contrast audit', () => {
  for (const [name, s] of schemes) {
    describe(name, () => {
      ...
      it('accent.info on surface.ground ≥ 3.3 (clinical graphics)', () => {
        // clinical cyan on #f4f6f8 is ~3.4:1 — badge/link graphics, not body text.
        expect(contrastRatio(s.accent.info, s.surface.ground)).toBeGreaterThanOrEqual(3.3);
      });

      it('accent.positive on surface.ground ≥ 3.4 (clinical graphics)', () => {
        expect(contrastRatio(s.accent.positive, s.surface.ground)).toBeGreaterThanOrEqual(3.4);
      });

      it('accent.negative on surface.ground ≥ 4.3 (clinical loss)', () => {
